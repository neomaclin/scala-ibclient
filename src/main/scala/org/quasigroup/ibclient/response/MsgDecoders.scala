...
object MsgDecoders:
  ...
  def decode(entry: Array[String])(using serverVersion: IBClient.ServerVersion): Either[Throwable, ResponseMsg] =
    entry.headOption
      .map(_.toInt)
      .fold(
        Right(
        ...
        case OPEN_ORDER =>
          OrderReader.processOpenOrder(using serverVersion).runA(entry.tail)
        case NEXT_VALID_ID =>
        ...
        case COMPLETED_ORDER =>
          OrderReader.processCompletedOrder(using serverVersion).runA(entry.tail)
